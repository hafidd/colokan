...
import { removeSchedule } from '../../../../app/services/device.service';
...
export default async function handler(
    ...
    if (!time || relay === null || relay === undefined) {
        return res.status(400).json({ message: 'time and relay are required' })
    }

    const relayIndex = Number(relay)
    const validTime = /^([01]\d|2[0-3]):[0-5]\d$/.test(time)

    if (!Number.isInteger(relayIndex) || relayIndex < 1 || !validTime) {
        return res.status(400).json({ message: 'invalid schedule input' })
    }

    const result = await removeSchedule(device.address, time, String(relayIndex))
